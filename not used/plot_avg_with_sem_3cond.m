...
fprintf('Making 3cond plots for %s\n', ratiotype);
   ...
   %Get list of genotypes present
   genotypes= fieldnames(avgratio_data);    

   %Cycle through genotype groups and plot 3cond plot for each
   for g = 1:length(genotypes)
        
       group_name = genotypes{g};
        ...
        for c = 1:3
            ...
            trace_legendHandles(g) = plot(all_secs, group_ratio, 'LineWidth', 1, 'Color', group_color);
    ...
    %first check if there are mutant genotypes code exists:
    ...
    if length(genotypes) > 1:
        ...
        trace_legendHandles = gobjects(2,3);
            ...
            for c = 1:3
                ...
                trace_legendHandles(1, c) = plot(all_secs, wt_ratio, 'LineWidth', 1, 'Color', wt_color);
                trace_legendHandles(2, c) = plot(all_secs, mt_ratio, 'LineWidth', 1, 'Color', mt_color1);
                ...
                legend([trace_legendHandles(1, c), trace_legendHandles(2, c), odour_patch, buffer_patch], ...
